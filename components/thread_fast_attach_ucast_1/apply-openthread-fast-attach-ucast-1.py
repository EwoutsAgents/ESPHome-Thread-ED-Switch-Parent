...
def find_patch_paths() -> list[Path]:
    ...
    for parent in candidates:
        patch_dir = parent / "patches/fast-attach-ucast-1"
        patches = [
            patch_dir / "openthread-preferred-parent-controller.patch",
            patch_dir / "openthread-fast-attach-pr13121.patch",
            patch_dir / "fast-attach-ucast-1-delta.patch",
        ]
        if all(patch.is_file() for patch in patches):
            return patches
    raise FileNotFoundError("Could not locate the fast-attach-ucast-1 patch set")
...
def apply_patch(root: Path, *, dry_run: bool = False) -> int:
    ...
    print(f"[thread_fast_attach_ucast_1] OpenThread root: {root}")
    print(f"[thread_fast_attach_ucast_1] patch_sha256={digest}")

    if not marker_file.exists():
        print(f"[thread_fast_attach_ucast_1][missing-file] {marker_file}")
        return 1
    if MARKER in marker_file.read_text(encoding="utf-8"):
        print("[thread_fast_attach_ucast_1][already] combined backport present")
        return 0
    ...
    if dry_run:
        ...
        print("[thread_fast_attach_ucast_1][dry-run] patch set found; ordered application deferred")
        return 0
    ...
    for patch_path in patch_paths:
        ...
        if result.returncode != 0:
            print(f"[thread_fast_attach_ucast_1] Failed applying {patch_path.name}")
            return result.returncode
    print("[thread_fast_attach_ucast_1][patched] combined selected-parent + Fast Attach backport applied")
    return 0
...
if _packages is not None:
    ...
    targets = [
        "$BUILD_DIR/src/esphome/components/thread_fast_attach_ucast_1/thread_fast_attach_ucast_1.cpp.o",
