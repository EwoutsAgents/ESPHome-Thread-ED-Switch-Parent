...
run_stock_batch() {
  ...
  local config="stock_test_devices_${router_count}routers.toml"
  local pattern="stock-${router_count}router-${CAMPAIGN_RUNS}runs-*"
  local runner_rc=0
  ...
  echo "Starting ${router_count}-router batch: $(date -Iseconds)"
  ./run_stock_test.sh --config "$config" --runs "$CAMPAIGN_RUNS" || runner_rc=$?
...
PY
  ...
  git -C "$CAMPAIGN_REPO" commit --only -m "test: record ${router_count}-router stock 50-run campaign" -- "$report_rel"
  git -C "$CAMPAIGN_REPO" push origin "$CAMPAIGN_BRANCH"

  echo "Completed, analyzed, committed, and pushed ${router_count}-router batch: $(date -Iseconds)"
