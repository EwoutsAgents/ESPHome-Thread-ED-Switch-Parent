...
echo "Variant: stock_low_power with ParentResponseDelay diagnostics"
...
run_stock_low_power_batch() {
  ...
  echo "Analyzing $batch_dir with random-delay subtraction"
...
PY
  ...
  git -C "$CAMPAIGN_REPO" commit --only \
    -m "test: record ${router_count}-router low-power stock 20-run delay campaign" \
    -- "$report_rel"
