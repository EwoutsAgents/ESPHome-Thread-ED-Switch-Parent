...
test -x ./run_stock_test.sh
test -x ./run_ucast_test.sh
test -x ./run_ucast_fastpr_test.sh
test -f scripts/analyze_test_logs.py

mapfile -t MCAST_CONFIGS < <(ls -1 mcast_test_devices_*routers.toml 2>/dev/null | sort -V)
mapfile -t STOCK_CONFIGS < <(ls -1 stock_test_devices_*routers.toml 2>/dev/null | sort -V)
mapfile -t UCAST_CONFIGS < <(ls -1 ucast_test_devices_*routers.toml 2>/dev/null | sort -V)
mapfile -t FASTPR_CONFIGS < <(ls -1 ucast_fastpr_test_devices_*routers.toml 2>/dev/null | sort -V)

if ((${#MCAST_CONFIGS[@]} == 0 && ${#STOCK_CONFIGS[@]} == 0 && ${#UCAST_CONFIGS[@]} == 0 && ${#FASTPR_CONFIGS[@]} == 0)); then
  echo "Systemic failure: no supported router configs found"
  exit 1
fi
...
commit_batch() {
  ...
  local variant="$2"
  local routers="$3"
  local msg="Add ${variant} ${routers}router 20-run batch"
  local push_log
  push_log="$(mktemp)"
...
run_batch() {
  ...
  local batch_rc=0
  local new_dir=""
  local pattern="${variant}-${routers}router-${RUNS}runs-*"
  ...
  echo "Command: ./$wrapper --config $config --runs $RUNS --clean-before-compile"
  if ! "./$wrapper" --config "$config" --runs "$RUNS" --clean-before-compile; then
    batch_rc=$?
...
for config in "${MCAST_CONFIGS[@]}"; do
  [[ "$config" =~ _([0-9]+)routers\.toml$ ]] || { echo "Systemic failure: bad config name $config"; exit 1; }
  run_batch "mcast" "run_mcast_test.sh" "$config" "${BASH_REMATCH[1]}"
done

for config in "${STOCK_CONFIGS[@]}"; do
  [[ "$config" =~ _([0-9]+)routers\.toml$ ]] || { echo "Systemic failure: bad config name $config"; exit 1; }
  run_batch "stock" "run_stock_test.sh" "$config" "${BASH_REMATCH[1]}"
done

for config in "${UCAST_CONFIGS[@]}"; do
  [[ "$config" =~ _([0-9]+)routers\.toml$ ]] || { echo "Systemic failure: bad config name $config"; exit 1; }
  run_batch "ucast" "run_ucast_test.sh" "$config" "${BASH_REMATCH[1]}"
done
