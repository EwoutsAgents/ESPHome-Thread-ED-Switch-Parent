...
if ((${#UCAST_CONFIGS[@]} == 0 && ${#FASTPR_CONFIGS[@]} == 0)); then
  echo "Systemic failure: no supported ucast configs found"
  exit 1
fi
...
run_batch() {
  ...
  if [[ "$config" =~ _([0-9]+)routers\.toml$ ]]; then
    routers="${BASH_REMATCH[1]}"
  else
    echo "Systemic failure: could not derive router count from $config"
    return 1
  fi

  mkdir -p "logs/$logs_subdir"
  mapfile -t BEFORE_DIRS < <(find "logs/$logs_subdir" -mindepth 1 -maxdepth 1 -type d -printf '%f\n' | sort)
  ...
  mapfile -t AFTER_DIRS < <(find "logs/$logs_subdir" -mindepth 1 -maxdepth 1 -type d -printf '%f\n' | sort)
  new_dir="$(comm -13 <(printf '%s\n' "${BEFORE_DIRS[@]}") <(printf '%s\n' "${AFTER_DIRS[@]}") | tail -n 1 || true)"
  if [[ -z "$new_dir" ]]; then
    echo "Systemic failure: no batch directory detected for variant=$variant config=$config"
    return 1
  fi

  local batch_rel="testing/logs/$logs_subdir/$new_dir"
  echo "Batch directory: $batch_rel"

  if ! python3 scripts/analyze_test_logs.py --logs-dir "logs/$logs_subdir/$new_dir" --write-markdown; then
...
for config in "${UCAST_CONFIGS[@]}"; do
  run_batch "ucast" "run_ucast_test.sh" "$config" "ucast"
done

for config in "${FASTPR_CONFIGS[@]}"; do
  run_batch "ucast_fastpr" "run_ucast_fastpr_test.sh" "$config" "ucast_fastpr"
done
