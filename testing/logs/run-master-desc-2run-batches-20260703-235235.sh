...
mapfile -t MCAST_CONFIGS < <(ls -1 mcast_test_devices_*routers.toml 2>/dev/null | sort -Vr)
mapfile -t STOCK_CONFIGS < <(ls -1 stock_test_devices_*routers.toml 2>/dev/null | sort -Vr)

if ((${#MCAST_CONFIGS[@]} == 0 && ${#STOCK_CONFIGS[@]} == 0)); then
  echo "Systemic failure: no supported master configs found"
  exit 1
fi
...
commit_batch() {
  ...
  local variant="$2"
  local routers="$3"
  local msg="Add ${variant} ${routers}router 2-run batch"
  local push_log
  push_log="$(mktemp)"
...
run_batch() {
  ...
  local batch_rc=0
  local new_dir=""
  local pattern="${variant}-${routers}router-2runs-*"
  ...
  echo "Command: ./$wrapper --config $config --runs 2 --clean-before-compile"
  if ! "./$wrapper" --config "$config" --runs 2 --clean-before-compile; then
    batch_rc=$?
