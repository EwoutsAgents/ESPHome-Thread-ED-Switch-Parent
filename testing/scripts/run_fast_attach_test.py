#!/usr/bin/env python3
"""Run the isolated Fast Attach ESPHome/OpenThread parent-switching test.
...
DEFAULT_CONFIG_NAMES = {
    "empty": "empty.yaml",
    "router1": "fast_attach_router_1.yaml",
    "child": "fast_attach_child.yaml",
    "router2": "fast_attach_router_2.yaml",
    "router3": "fast_attach_router_3.yaml",
    "router4": "fast_attach_router_4.yaml",
}
...
UPSTREAM_PR_HEAD = "61034b8a7e776695d1afe30338b3f36d24733127"
...
def firmware_environment(settings: Settings, *, phase: str) -> dict[str, Any]:
    patch_path = settings.testing_dir.parent / "patches/fast-attach/openthread-fast-attach-pr13121.patch"
    return {
        "phase": phase,
        "variant": settings.variant_name,
        "platformio_core_dir": str(settings.platformio_core_dir),
        "platformio_packages_dir": str(settings.platformio_packages_dir),
        "openthread_pr": "https://github.com/openthread/openthread/pull/13121",
        "openthread_pr_head": UPSTREAM_PR_HEAD,
        "vendored_openthread_revision": VENDORED_OPENTHREAD_REVISION,
        "fast_attach_patch": str(patch_path),
        "fast_attach_patch_sha256": hashlib.sha256(patch_path.read_bytes()).hexdigest(),
        "fast_attach_compile_option": "OPENTHREAD_CONFIG_MLE_FAST_ATTACH_ENABLE=1",
    }
...
class RunTracker:
    def __init__(
        ...
        *,
        ...
        self.manifest_path = self.settings.run_logs_dir / f"fast_attach_test_manifest_{self.settings.run_logs_dir.name}.json"
...
def load_settings(args: argparse.Namespace) -> Settings:
    ...
    if not config_file.exists():
        ...
        if not example.exists():
            example = config_file.parent / "fast_attach_test_devices.example.toml"
    ...
    variant_name = str(variant_raw.get("name", "fast-attach")).strip()
    if variant_name != "fast-attach":
        raise SystemExit(f"Unsupported variant `{variant_name}`. Only `fast-attach` is supported by this runner.")
    ...
    if max_router_number < 2 or f"router{max_router_number}" not in config_names:
        raise SystemExit(
            f"[variant].n_routers must reference an available fast_attach_router_<n>.yaml variation. "
...
def log_file_path(settings: Settings, logical_name: str) -> Path:
    ...
    return settings.run_logs_dir / f"fast_attach_{logical_name}_{stamp}.log"
...
def start_sniffer_capture(
    ...
    *,
    ...
    stamp = settings.run_logs_dir.name
    log_path = settings.run_logs_dir / f"fast_attach_sniffer_{stamp}.log"
...
def pull_sniffer_pcap(
    ...
    *,
    ...
    stamp = settings.run_logs_dir.name
    local_pcap = settings.run_logs_dir / f"fast_attach_sniffer_{stamp}.pcapng"
    host = sniffer_remote_host(settings)
    if host:
...
def router_execution_plan(settings: Settings) -> list[dict[str, str]]:
    """Return routers that participate in the Fast Attach run."""
    plan = [
...
def run_timed_sequence(
    ...
    *,
    ...
    log("Starting dynamic-parent Fast Attach upload/logging sequence. Upload-only commands are used from here onward.")
    ...
    try:
        ...
        sleep_step(
            ...
            "all Fast Attach routers flashed/logging; fixed router-settling delay before flashing child",
            dry_run=dry_run,
            manifest=manifest,
            tracker=tracker,
        )
        ...
        sleep_step(
            ...
            "child flashed/logging; wait for natural initial attach before detecting current parent",
            dry_run=dry_run,
            manifest=manifest,
            tracker=tracker,
        )
...
def write_manifest(
    ...
    *,
    ...
    settings.run_logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = settings.run_logs_dir.name
    path = settings.run_logs_dir / f"fast_attach_test_manifest_{stamp}.json"
    if path.exists():
        return path
    payload = {
        "created_utc": now_utc_iso(),
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Fast Attach ESPHome/OpenThread parent-switching test.")
    parser.add_argument("--config", default="fast_attach_test_devices.toml", help="Path to device/config TOML file.")
