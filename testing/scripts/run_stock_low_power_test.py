#!/usr/bin/env python3
"""Run the stock-low-power ESPHome/OpenThread parent-switching test.
...
DEFAULT_CONFIG_NAMES = {
    "empty": "empty.yaml",
    "router1": "stock_low_power_router_1.yaml",
    "child": "stock_low_power_child.yaml",
    "router2": "stock_low_power_router_2.yaml",
    "router3": "stock_low_power_router_3.yaml",
    "router4": "stock_low_power_router_4.yaml",
}
ROUTER_OUTPUT_POWER_DBM = {"router1": 0, "router2": 0, "router3": -15, "router4": -15}
...
class Settings:
    ...
    variant_name: str = "stock-low-power"
...
def firmware_environment(settings: Settings, *, phase: str) -> dict[str, Any]:
    return {
        ...
        "platformio_core_dir": str(settings.platformio_core_dir),
        "platformio_packages_dir": str(settings.platformio_packages_dir),
        "router_output_power_dbm": ROUTER_OUTPUT_POWER_DBM,
    }
...
class RunTracker:
    def __init__(
        ...
        *,
        ...
        self.manifest_path = self.settings.run_logs_dir / f"stock-low-power_test_manifest_{self.settings.run_logs_dir.name}.json"
...
def load_settings(args: argparse.Namespace) -> Settings:
    ...
    if not config_file.exists():
        ...
        if not example.exists():
            example = config_file.parent / "stock_low_power_test_devices_4routers.toml"
    ...
    variant_name = str(variant_raw.get("name", "stock-low-power")).strip()
    if variant_name != "stock-low-power":
        raise SystemExit(f"Unsupported variant `{variant_name}`. Only `stock-low-power` is supported by this runner.")
    ...
    if max_router_number < 2 or f"router{max_router_number}" not in config_names:
        raise SystemExit(
            f"[variant].n_routers must reference an available stock_low_power_router_<n>.yaml variation. "
...
def log_file_path(settings: Settings, logical_name: str) -> Path:
    ...
    return settings.run_logs_dir / f"stock-low-power_{logical_name}_{stamp}.log"
...
def start_sniffer_capture(
    ...
    *,
    ...
    stamp = settings.run_logs_dir.name
    log_path = settings.run_logs_dir / f"stock-low-power_sniffer_{stamp}.log"
...
def pull_sniffer_pcap(
    ...
    *,
    ...
    stamp = settings.run_logs_dir.name
    local_pcap = settings.run_logs_dir / f"stock-low-power_sniffer_{stamp}.pcapng"
    host = sniffer_remote_host(settings)
    if host:
...
def router_execution_plan(settings: Settings) -> list[dict[str, str]]:
    """Return routers that participate in the stock-low-power run."""
    plan = [
...
def run_timed_sequence(
    ...
    *,
    ...
    log("Starting dynamic-parent stock-low-power sequence. Upload-only commands are used from here onward.")
    ...
    try:
        ...
        sleep_step(
            ...
            "all stock-low-power routers flashed/logging; fixed router-settling delay before flashing child",
            dry_run=dry_run,
            manifest=manifest,
            tracker=tracker,
        )
        ...
        sleep_step(
            ...
            "child flashed/logging; wait for natural stock-low-power attach before detecting current parent",
            dry_run=dry_run,
            manifest=manifest,
            tracker=tracker,
        )
        ...
        detection_details = {
            ...
            "router_output_power_dbm": ROUTER_OUTPUT_POWER_DBM,
            "detected_parent_output_power_dbm": (
                ROUTER_OUTPUT_POWER_DBM.get(parent_match["logical_name"]) if parent_match else None
            ),
            "initial_parent_matches_expected_high_power": (
                ROUTER_OUTPUT_POWER_DBM.get(parent_match["logical_name"]) == 0 if parent_match else None
            ),
        }
        ...
        removal_details = {
            **detection_details,
            ...
            "expected_replacement_high_power_router": (
                "router2" if parent_plan["logical_name"] == "router1" else
                "router1" if parent_plan["logical_name"] == "router2" else None
            ),
        }
...
def write_manifest(
    ...
    *,
    ...
    settings.run_logs_dir.mkdir(parents=True, exist_ok=True)
    stamp = settings.run_logs_dir.name
    path = settings.run_logs_dir / f"stock-low-power_test_manifest_{stamp}.json"
    if path.exists():
        return path
    payload = {
        "created_utc": now_utc_iso(),
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run stock-low-power ESPHome/OpenThread parent-switching test.")
    parser.add_argument("--config", default="stock_low_power_test_devices_4routers.toml", help="Path to device/config TOML file.")
