...
8. Wait for `child_attach_seconds` seconds to allow the child to attach naturally to one of the available routers.
...
12. If the child's current was safe to remove, wait `after_parent_removed_seconds` seconds while continuing to record packets and device logs.
